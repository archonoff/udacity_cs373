...
def run(grid, goal, spath, params, printflag=False, speed=0.1, timeout=1000, plot=False, collisions_coefficient=100, steps_coefficient=10):
    myrobot = robot()
    myrobot.set(0., 0., 0.)
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        if plot:
            robot_positions.append(estimate[0:2])
            robot_positions2.append((myrobot.x, myrobot.y))
            particles_positions.append([(particle.x, particle.y) for particle in filter.data])

        while True:
            dx = spath[current_segment + 1][0] - spath[current_segment][0]
            dy = spath[current_segment + 1][1] - spath[current_segment][1]
            rx = estimate[0] - spath[current_segment][0]
            ry = estimate[1] - spath[current_segment][1]
            seg_len = sqrt(dx ** 2 + dy ** 2)
            u = (rx * dx + ry * dy) / seg_len
            if u > 1:
                current_segment += 1
            else:
                cte = (ry * dx - rx * dy) / seg_len
                break
        total_cte += abs(cte)
    ...
    if plot:
        ...
        # ani.save('robot_animation.gif', fps=)
        # ani.save('robot_animation.gif', writer='imagemagick', fps=3)
        plt.show()

    if plot:
        return [myrobot.check_goal(goal), myrobot.num_collisions, myrobot.num_steps]
    else:
        return total_cte + collisions_coefficient * myrobot.num_collisions + steps_coefficient * myrobot.num_steps
...
def main(grid, init, goal, steering_noise, distance_noise, measurement_noise,
         weight_data, weight_smooth, p_gain, d_gain, plot=False):
    path = plan(grid, init, goal)
    path.astar()
    path.smooth(weight_data, weight_smooth)

    if plot:
        plt.plot(zip(*path.path), label='Computed path', linestyle='--')       # вычисленный путь
        plt.plot(zip(*path.spath), label='Smoothed path', linestyle='--')       # сглаженный путь
        obstacles = []
        for x in range(len(grid)):
            for y in range(len(grid[0])):
                if grid[x][y] == 1:
                    obstacles.append((x, y))
        plt.scatter(*tuple(zip(*obstacles)), s=3100, marker='s')       # препятствия

    return run(grid, goal, path.spath, [p_gain, d_gain], plot=plot)
...
def twiddle_run(p, n_launches=10):
    print(p)
    weight_data, weight_smooth, p_gain, d_gain = p
    error = 0
    for launch in range(n_launches):
        total_cte = main(grid, init, goal, steering_noise, distance_noise, measurement_noise, weight_data, weight_smooth, p_gain, d_gain, plot=False)
        error += total_cte
    return error / n_launches
...
def twiddle(tol=0.2):
    ...
    p = [weight_data, weight_smooth, p_gain, d_gain]        # PID components
    dp = [0.05, 0.05, 0.5, 0.5]
    best_err = get_err(p)
...
print(main(grid, init, goal, steering_noise, distance_noise, measurement_noise, weight_data, weight_smooth, p_gain, d_gain, plot=True))
# p, best_err = twiddle()
# print(p)
# print(best_err)
