...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    Omega = matrix()
    Omega.zero(dim, dim)
    Omega.value[0][0] = 1.0     # x
    Omega.value[1][1] = 1.0     # y
    ...
    for k in range(len(data)):
        ...
        # расширить матрицы
        Omega = Omega.expand(dimx=(dim+2), dimy=(dim+2), list1=[m for m in range(dim+2) if m not in (2, 3)])
        Xi = Xi.expand(dimx=(dim+2), dimy=1, list1=[m for m in range(dim+2) if m not in (2, 3)], list2=[0])

        # заполнить расширенную матрицу
        # integrate the measurements
        for i in range(len(measurement)):
            # m is the index of the landmark coordinate in the matrix/vector
        ...
        # ужать матрицу и перейти на следующий цикл
        Omega_prime = Omega.take(list1=[m for m in range(2, dim+2)])
        A = Omega.take(list1=[0, 1], list2=[m for m in range(2, dim+2)])
        B = Omega.take(list1=[0, 1])

        Xi_prime = Xi.take(list1=[m for m in range(2, dim+2)], list2=[0])
        C = Xi.take(list1=[0, 1], list2=[0])
...
plt.scatter([c[0] for i, c in enumerate(result.value[N*2:]) if i % 2 == 0],
            [c[0] for i, c in enumerate(result.value[N*2:]) if i % 2 == 1],
            label='SLAM landmarks')
plt.scatter(result.value[(N-1)*2],
            result.value[(N-1)*2 + 1],
            label='SLAM final position')
'''
...
# omega - это inverse covariance matrix
print(result[1].show())
print(result[1].inverse().show())

plt.scatter([c[0] for i, c in enumerate(result[0].value[2:]) if i % 2 == 0],
            [c[0] for i, c in enumerate(result[0].value[2:]) if i % 2 == 1],
            label='Online SLAM landmarks')
plt.scatter(result[0].value[0],
            result[0].value[1],
            label='Online SLAM final position')
# '''
