...
def next_move(hunter_position, hunter_heading, target_measurement, max_distance, OTHER=None, max_steps=15):
    ...
    if OTHER is None:
        ...
        OTHER = {
            ...
            'current_target_position': target_measurement,
    ...
    current_target_position = kalman_filter.get_position()
    ...
    OTHER['current_target_position'] = current_target_position
...
def build_graphs(hunter_bot, target_bot, next_move_fcn, OTHER=None):
    max_distance = 0.98 * target_bot.distance  # 0.98 is an example. It will change.

    target_positions_x = []
    target_measurements_x = []
    ...
    for i in range(500):
        ...
        # position_guess = OTHER['meeting_position']
        # next_target_guess = OTHER['target_position']
        current_target_position = OTHER['current_target_position']
        ...
        target_positions_x.append(target_position[0])
        target_measurements_x.append(target_measurement[0])
        filtered_positions_x.append(current_target_position[0])

    plt.figure(figsize=(15, 6))
    plt.plot(target_positions_x, label='Истиное положение')
    plt.plot(target_measurements_x, label='Измеренное положение')
    plt.plot(filtered_positions_x, label='Отфильтрованное положение')
    plt.legend()
    plt.show()
...
if __name__ == '__main__':
    ...
    # print(build_graphs(hunter, target, next_move))
